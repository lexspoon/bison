...
# b4_symbol_type_register_if_needed(SYMBOL-NUM)
# ---------------------------------------------
# Register a member of the union for the given SYMBOL-NUM
# unless it is a symbol for a terminal and Bison is generating
# the lexer implementation. In that case, don't put it in the
# union, because all terminals will be tracked with a single
# yyterminal member of the union.
m4_define([b4_symbol_type_register_if_needed],
[b4_lex_if(
  [b4_symbol_if($1, [is_token], [], [
    b4_symbol_type_register($1)])],
  [b4_symbol_type_register($1)])])
...
[b4_symbol_if([$1], [has_type],
              [m4_map([b4_symbol_type_register_if_needed], [$@])])
])
...
{
  int res = 0;
]dnl Don't subtract one from last_column if the lexer was generated by
dnl Bison.  In an attempt to help the developer, Bison's generated
dnl lexers use the convention that last_column points to the last
dnl character rather than one past the last character. This convention
dnl matches the way column numbers are communicated to human users,
dnl which after all is the main purpose of tracking column numbers to
dnl begin with. As well, this convention makes last_column match
dnl last_line, which itself points to the last line of the token
dnl rather than one past.
b4_lex_if(
[  int end_col = yylocp->last_column;],
